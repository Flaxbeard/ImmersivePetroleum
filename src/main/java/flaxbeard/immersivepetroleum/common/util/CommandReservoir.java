		...
		if(args.length > 1)
		{
			...
			switch(args[1])
			{
				case "list":
					...
					break;
				case "get":
					OilWorldInfo info = PumpjackHandler.getOilWorldInfo(sender.getEntityWorld(), coords.chunkXPos, coords.chunkZPos);
					...
					for (String g : h.split("<br>"))
						sender.addChatMessage(new TextComponentString(g));
					break;
				case "set":
					info = PumpjackHandler.getOilWorldInfo(sender.getEntityWorld(), coords.chunkXPos, coords.chunkZPos);
					
					if (args.length < 3)
					{
						sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".set.clear", (info.overrideType != null ? info.overrideType.name : "null")));
					...
					if (reservoir == null)
					{
						sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".set.invalidReservoir", args[2]));
						return;
					}
					info.overrideType = reservoir;
					sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".set.sucess", reservoir.name));
				...
				case "setAmount":
					...
					{
						sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".setAmount.NFE", args[2].trim()));
						return;
					}
					amount = Math.min(info.capacity, amount);
					amount = Math.max(0, amount);
					info.current = amount;
					sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".setAmount.sucess", Integer.toString(amount)));
					IPSaveData.setDirty(sender.getEntityWorld().provider.getDimension());
					break;
				case "setCapacity":
					info = PumpjackHandler.getOilWorldInfo(sender.getEntityWorld(), coords.chunkXPos, coords.chunkZPos);
					...
					{
						sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".setCapacity.NFE", args[2].trim()));
						return;
					}
					amount = Math.max(0, amount);
					info.capacity = amount;
					sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + getIdent() + ".setCapacity.sucess", Integer.toString(amount)));
					IPSaveData.setDirty(sender.getEntityWorld().provider.getDimension());
					break;
				default:
