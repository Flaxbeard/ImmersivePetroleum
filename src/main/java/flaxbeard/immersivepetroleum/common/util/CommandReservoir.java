		...
		if(args.length > 1)
		{
			...
			switch(args[1])
			{
				...
				case "setAmount":
					...
					info.current = amount;
					sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + ".setAmount.sucess", Integer.toString(amount)));
					IPSaveData.setDirty(sender.getEntityWorld().provider.getDimension());
					break;
				case "setCapacity":
					info = PumpjackHandler.getOilWorldInfo(sender.getEntityWorld(), coords.chunkXPos, coords.chunkZPos);
					...
					sender.addChatMessage(new TextComponentTranslation("chat.immersivepetroleum.command." + ".setCapacity.sucess", Integer.toString(amount)));
					IPSaveData.setDirty(sender.getEntityWorld().provider.getDimension());
					break;
				default:
