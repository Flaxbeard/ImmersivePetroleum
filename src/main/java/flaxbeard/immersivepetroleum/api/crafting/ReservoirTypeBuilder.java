...
public class ReservoirTypeBuilder extends IEFinishedRecipe<ReservoirTypeBuilder>{
	...
	/**
	 * Sets minimum <code>amount</code> of fluid for this Reservoir.
	 * <code><pre>
	 * 1.000 = 1 Bucket
	 * 0.001 = 1 Millibucket
	 * </pre></code>
	...
	/**
	 * Replenish <code>amount</code> per tick.
	 * <code><pre>
	 * 1.000 = 1 Bucket
	 * 0.001 = 1 Millibucket
	 * </pre></code>
		...
		if(isBlacklist){
			if(dimensions!=null && dimensions.length>0){
				...
				for(ResourceLocation rl:dimensions)
					if(rl!=null)
						if(!this.dimBlacklist.contains(new JsonPrimitive(rl.toString()))) // Avoid duplicates
			...
			if(dimensions!=null && dimensions.length>0){
				...
				for(ResourceLocation rl:dimensions)
					if(rl!=null)
						if(!this.dimWhitelist.contains(new JsonPrimitive(rl.toString()))) // Avoid duplicates
		...
		if(isBlacklist){
			if(biomes!=null && biomes.length>0){
				...
				for(ResourceLocation rl:biomes)
					if(rl!=null)
						if(!this.bioBlacklist.contains(new JsonPrimitive(rl.toString()))) // Avoid duplicates
			...
			if(biomes!=null && biomes.length>0){
				...
				for(ResourceLocation rl:biomes)
					if(rl!=null)
						if(!this.bioWhitelist.contains(new JsonPrimitive(rl.toString()))) // Avoid duplicates
